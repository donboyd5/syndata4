...
rungroup <- function(group.ind, idfile, recipe, puf.vnames, mrgdf){
  print(group.ind)
  a <- proc.time()
...
a <- proc.time()
l_ply(1:max(idfile$group), .fun=rungroup, idfile=idfile, recipe=recipe, puf.vnames=puf.vnames, mrgdf=mrgdf, .progress="text", .parallel=FALSE, .paropts=popts)
b <- proc.time()
b - a # 
