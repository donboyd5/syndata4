...
runsyn <- function(i, grouplist, groupnames, sampfrac=1, synname, m=1){
  # get easier names
...
runsyn_mdp <- function(df){
  ...
  pred.mat.corrected <- correct_predictor_matrix(pufsyn, visit.sequence)
...
inputs <- list()
inputs$synname <- synname
inputs$m <- m
inputs$sampfrac <- sampfrac
inputs$syngroups <- syngroups
inputs$synrecipe.use <- get_synrecipe("recipe7")
inputs$vargroups <- get_vargroups(inputs$synrecipe.use)
inputs$methods <- get_methods(vargroups)
...
# xport <- c("globals", "puf.base", "visit.sequence", "methods", 
#            "idvars", "xvars.keep", "xvars.drop", "synvars.vs",
#            "pred.mat.corrected")
# xport <- c("globals", "puf.base", "inputs", "correct_predictor_matrix")
...
a <- proc.time()
stack <- ldply(syngroups, runsyn, mslist, msnames, sampfrac, synname, m, .progress="text", .parallel=FALSE, .paropts=popts)
b <- proc.time()
b - a # 3.5 hours
...
results <- flights %>% 
  partition(dest) %>% 
  summarise(est_travel_time=mean(air_time, na.rm=TRUE)) %>% 
  collect() %>% 
...
f <- function(df){
  # df$est_travel_time <- mean(df$air_time, na.rm=TRUE)
  df2 <- tibble::tibble(est_travel_time=mean(df$air_time, na.rm=TRUE))
  return(df2)
}
cluster <- get_default_cluster()
cluster_assign_value(cluster, 'f', f) # Register function -- reregister if function changes
a <- proc.time()
results2 <- flights %>% 
  partition(dest) %>% 
  do(f(.)) %>%
...
# cluster_assign_value(cluster, 'f', f) # Register function -- reregister if function changes
cluster_copy(cluster, f) # Register function -- reregister if function changes
cluster_ls(cluster) # Check registered items
cluster_get(cluster, 'f') # Get the item
# cluster_rm(cluster, c('f')) # Unregister function [or other items]
a <- proc.time()
results3 <- flights %>% 
  partition(dest) %>% 
  do(f(.)) %>%
...
df <- readRDS(paste0(globals$tc.dir, "pufsyn_stack/", "synthpop9_MARS1.rds"))
names(df)
names(df$stack)
length(unique(df$stack$e00200))
# 1179 --> 3323 serial, no density --> density
# 1205 --> 3323 parallel
...
write_csv(output$stack, paste0(globals$synd, synname, "_stack.csv"))
write_csv(output$stack %>% filter(ftype=="puf"), paste0(globals$synd, synname, ".csv"))
