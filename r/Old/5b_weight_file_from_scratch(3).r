...
recipe <- get_recipe_long(get_weighting_recipe("recipe4")) %>%
...
rungroup <- function(group.ind){
  ...
  getinplist <- function(syn, recipe.use){
    ...
    inputs$recipe <- recipe.use
    # inputs$synsub <- syn[, unique(inputs$recipe$vname)] %>% mutate(wt=1)
    inputs$synsub <- syn %>% select(unique(inputs$recipe$vname)) %>% mutate(wt=1)
    synlong <- inputs$synsub %>%
...
n <- 45
# obj.vals <- laply(1:n, function(i) optlist[[i]]$result$par) # this used to work but doesn't now??
obj.vals <- {lapply(1:n, function(i) optlist[[i]]$result$par)} %>% unlist
ht(obj.vals)
...
obj.val <- laply(1:n, function(i) optlist[[i]]$result$value)
obj.val %>% round(3)
count(idfile, mgroup, ftype, group) %>% spread(ftype, n) %>% mutate(diff=syn - puf, sum=puf + syn)

# match the groups up with their obj value
objdf <- idfile %>%
  group_by(ftype, group, mgroup) %>%
  summarise(n=n()) %>% 
  spread(ftype, n) %>% 
  mutate(diff=syn - puf, sum=puf + syn) %>%
  ungroup %>%
  arrange(group) %>%
  mutate(obj=obj.val)
objdf %>% kable(digits=2)
