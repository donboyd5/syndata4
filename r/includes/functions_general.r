...
get_puf_vnames <- function(){
  ...
  readRDS(file.path(PROJHOME, "data", "puf.vnames.rds"))
}
...
get_pufbase_sums <- function(){
  ...
  readRDS(file.path(PROJHOME, "data", "puf.base.sums.rds"))
...
tc.wincmd <- function(tc.fn, tc.dir, tc.cli, taxyear=2013, taxplan.fn=NULL, taxplans.dir=NULL){
  ...
  # NOTE: system2 also works and may be more intuitive -- for that, we create two items:
  #   a command (without arguments), and
  #   a character string of arguments
  # For that, I do something such as the following:
  ...
  tc.infile.fullpath <- shQuote(paste0(tc.dir, tc.fn))
  # tc.outdir <- shQuote(str_sub(tc.dir, 1, -1)) # must remove trailing "/"
  tc.outdir <- shQuote(tc.dir)
  
  taxplanstring <- NULL
  if(!is.null(taxplan.fn)) taxplanstring <- paste0("--reform", " ", shQuote(paste0(taxplans.dir, taxplan.fn)))
